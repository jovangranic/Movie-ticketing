...
export class RezervacijaDialogComponent {
  ...
  public add() {
    this.service.addRezervacija(this.data).subscribe((data) => {
      this.snackBar.open(
        `Uspesno dodata rezervacija sa ID: ${data.id}`,
        `U redu`,
        { duration: 2500 }
      );
    }),
  ...
  public delete() {
    this.service.deleteRezervacija(this.data.id).subscribe((data) => {
