...
export class SalaDialogComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.bioskopService.getAllBioskop().subscribe(
      (data) => {
        this.bioskopi = data;
      }
    )
...
public add() {
  this.service.addSala(this.data).subscribe(
    (data) => {
      this.snackBar.open(`Uspešno dodata sala sa ID: ${data.id}`, `U redu`, {duration:2500});
    }
  ),
  (error:Error) => {
...
public update() {
  this.service.updateSala(this.data).subscribe(
    (data) => {
      this.snackBar.open(`Sala sa ID: ${data.id} je uspešno ažurirana`, `U redu`, {duration:2500});
    }
  ),
  (error:Error) => {
    console.log(error.name + ' ' + error.message);
...
public delete() {
  this.service.deleteSala(this.data.id).subscribe(
    (data) => {
      this.snackBar.open(`Sala sa ID: ${data.id} je uspešno obrisana`, `U redu`, {duration:2500});
    }
  ),
  (error:Error) => {
    console.log(error.name + ' ' + error.message);
